...
async def cancel_order(account_id: str, order_id: str) -> dict:
    return {
        "data": controller.system.customer_cancel_order(account_id, order_id)
    }
...
async def cancel_food(restaurant_account_id: str, order_id: str, food_name: str, string: str) -> dict:
    return {
        "data": controller.system.restaurant_cancel_order(restaurant_account_id, order_id, food_name, string)
    }

@app.post("/cancel_by_rider", tags = ["Cancel"])
async def cancel_rider(rider_account_id: str, order_id: str) -> dict:
    return {
        "data": controller.system.rider_cancel_order(rider_account_id, order_id)
    }